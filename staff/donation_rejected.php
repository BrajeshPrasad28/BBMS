<?php
 ...
 $sql2 = "SELECT * from donarregistration as donordetail INNER JOIN donateblood as  donateblood ON donordetail.id = donateblood.donarreg_id";

 $sql = "SELECT * from donarregistration as donordetail INNER JOIN donateblood as donateblood  ON  donordetail.id  = donateblood.donarreg_id  WHERE status='accepted'";
 if(isset($_POST['accepted'])){
   $donarreg_id = $_POST['donarreg_id'];
   $query = mysqli_query($con,"UPDATE donateblood SET status='accepted' WHERE donarreg_id = '$donarreg_id'");
   if($query){
  ...
  if(isset($_POST['denied'])){
    $donarreg_id = $_POST['donarreg_id'];
    $query = mysqli_query($con,"UPDATE donateblood SET status='rejected' WHERE donarreg_id = '$donarreg_id'");
    if($query){
        echo "<script> alert('Successfully Updated')</script>";
    }else{
      echo "Not Updated, reason: ".mysqli_error($con);
