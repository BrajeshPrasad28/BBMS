<?php
 ...
 $sql2 = "SELECT * from donarregistration as donordetail INNER JOIN donateblood as  donateblood ON donordetail.id = donateblood.donarreg_id WHERE status='accepted'";

$sql = "SELECT * from donarregistration as donordetail INNER JOIN donateblood as donateblood  ON  donordetail.id  = donateblood.donarreg_id ";
...
<?php
      ...
      while ($a = mysqli_fetch_array($query)) {
        ...
        echo "<td>".$a['status']."</td>";
        echo "<td>".$a['checkup_status']."</td>";
        echo "<td> <form action='staff_blood_checkup.php' method='post' style='display: inline-block;'> <input type='hidden' name='blooddonation_id' value='".$a['id']."' > <button  type='submit' name='donate'  style='margin:0px 5px; display:inline-block;  float:left;' class='btn btn-primary'>Take Donation</button>  </form> </td>";
        echo "  </tr>";
        }
    ?>
