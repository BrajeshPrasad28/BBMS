<?php
 ...
 $sql2 = "SELECT * from donarregistration as donordetail INNER JOIN chech_up as  check_up ON donordetail.id = check_up.id WHERE status='elligible'";

$sql = "SELECT * from donarregistration as donordetail INNER JOIN chech_up as  check_up ON donordetail.id = check_up.id ";
...
<?php
      ...
      while ($a = mysqli_fetch_array($query)) {
        ...
        echo "<td>".$a['status']."</td>";
        echo "<td>".$a['checkup_id']."</td>";
        echo "<td> <form action='staff_takedonation.php' method='post' style='display: inline-block;'> <input type='hidden' name='blooddonation_id' value='".$a['id']."' > <button  type='submit' name='donate'  style='margin:0px 5px; display:inline-block;  float:left;' class='btn btn-primary'>Take Donation</button>  </form> </td>";
        echo "  </tr>";
        }
    ?>
